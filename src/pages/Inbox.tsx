...
const InboxPage = () => {
  ...
  // Quick actions
  const handleReply = (orderId: string) => {
    navigate(`/feedback-room/${orderId}`);
  };
  ...
  const handlePay = (orderId: string) => {
    navigate(`/dashboard?tab=billing&orderId=${orderId}`);
  };
    ...
    <ProtectedRoute>
      <PageLayout maxWidth="max-w-4xl">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {t.inbox?.subtitle ?? "Everything that needs your attention"}
            </p>
          </div>
          <Button
            variant="ghost"
        ...
        <div className="flex gap-2 flex-wrap mb-4">
          {(["chat", "approval", "delivery", "invoice"] as InboxItemType[]).map(
            (type) =>
              counts[type] > 0 && (
                <Badge
                  key={type}
                  variant="outline"
                  className={cn("text-xs font-medium", TYPE_STYLES[type].badge)}
                >
        ...
        {isLoading ? (
          ...
          <EmptyState
            icon={InboxIcon}
            title={t.inbox?.allCaughtUp ?? "All caught up!"}
            description={t.inbox?.noItems ?? "No pending items need your attention right now."}
          />
        ) : (
                          ...
                          <div className="flex flex-col gap-1.5 shrink-0">
                            {item.type === "chat" && (
                              <Button
                                ...
                                onClick={() => handleReply(item.orderId)}
                              >
                                <Reply className="h-3 w-3 ltr:mr-1 rtl:ml-1" />
                                {t.inbox?.reply ?? "Reply"}
                              </Button>
                            )}
                            {item.type === "approval" && (
                            ...
                            {item.type === "delivery" && (
                              <Button
                                size="sm"
                                className="h-8 text-xs"
                                onClick={() => handleConfirmDelivery(item.orderId)}
                              >
                                <PackageCheck className="h-3 w-3 ltr:mr-1 rtl:ml-1" />
                                {t.inbox?.confirmDelivery ?? "Confirm"}
                              </Button>
                            )}
                            {item.type === "invoice" && (
                              <Button
                                ...
                                variant="outline"
                                className="h-8 text-xs"
                                onClick={() => handlePay(item.orderId)}
                              >
